  ...
  return (
    <div className="w-full max-w-4xl p-6">
      ...
      <div className="mb-6 relative">
        ...
        />
        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[var(--accent-color)]" size={20} />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 h-[400px] overflow-y-auto">
        {filteredTemplates.map(template => (
          ...
          >
            <h3 className="text-lg font-semibold text-center truncate">{template.name}</h3>
          </div>
        ))}
      </div>
